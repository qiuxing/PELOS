...
# stat: vector of statistics
{
    ...
    if(H0 == "mean"){
      ss0 = rowSums((y-matrix(rep(mu,m),,m))^2,na.rm=TRUE)
      ss1 = rowSums((y-yfit)^2,na.rm=TRUE)
      stat = (ss0-ss1)/(ss1+delta)
      ## ss = sort(stat,index.return=TRUE)
      ...
      for(i in 1:B){
        ...
        ss1bt = rowSums((samp-sampfit)^2,na.rm=TRUE)
        bt = (ss0-ss1bt)/(ss1bt+delta)
        nbt = nbt+hist(bt,breaks=edge,plot=F)$count
      }
    }else{
      ss0 = rowSums((y-matrix(rep(yfit[,1],m),,m))^2,na.rm=TRUE)
      ss1 = rowSums((y-yfit)^2,na.rm=TRUE)
      stat = (ss0-ss1)/(ss1+delta)
      ss = sort(stat,index.return=TRUE)
      edge = c(min(stat)-100,ss,max(stat)+100)
      ...
      for(i in 1:B){
        ...
        samp = y[,unlist(ind)]
        sampfit = PCApred(res,samp,tt,tmprr,miss)$yfit_orig
        ss0bt = rowSums((samp-matrix(rep(sampfit[,1],m),,m))^2,na.rm=TRUE)
        ss1bt = rowSums((samp-sampfit)^2,na.rm=TRUE)
    ...
    ## p = p[sort(oo,index.return=TRUE)$ix]
    p = p[order(oo)]
